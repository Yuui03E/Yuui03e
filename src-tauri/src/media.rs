...
fn get_cache_dir() -> PathBuf {
    if let Ok(appdata) = std::env::var("APPDATA") {
        Path::new(&appdata).join("com.yuui.app").join("cache")
    } else {
        PathBuf::from("cache")
    }
}
...
async fn get_ffmpeg_path(pool: &SqlitePool) -> String {
    let row: Option<(String,)> = sqlx::query_as("SELECT value FROM settings WHERE key = 'ffmpeg_path'")
        .fetch_optional(pool)
        .await
        .unwrap_or(None);
    row.map(|r| r.0).filter(|s| !s.trim().is_empty()).unwrap_or_else(|| "ffmpeg".to_string())
}

async fn get_ffprobe_path(pool: &SqlitePool) -> String {
    let row: Option<(String,)> = sqlx::query_as("SELECT value FROM settings WHERE key = 'ffprobe_path'")
        .fetch_optional(pool)
        .await
        .unwrap_or(None);
    row.map(|r| r.0).filter(|s| !s.trim().is_empty()).unwrap_or_else(|| "ffprobe".to_string())
}
...
pub fn start_preview_worker(pool: SqlitePool) {
