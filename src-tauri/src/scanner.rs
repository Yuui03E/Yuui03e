...
fn is_generic_folder(name: &str) -> bool {
    ...
    for pattern in &generic_patterns {
        ...
        if lower.starts_with(pattern) {
            let next_char = lower.chars().nth(pattern.chars().count());
    ...
    // Language code folders (allowlist of explicit names)
    let lang_folders = [
        "eng", "english", "jpn", "japanese", "sub", "subs", "subtitle", "subtitles",
        "dub", "dubs", "raw", "raws", "ger", "fre", "spa", "ita", "rus", "chi", "kor",
        "zho", "multi", "dual", "dual-audio", "dual audio",
    ];
...
fn find_best_folder_title(file_path: &str, scan_roots: &[String]) -> Option<String> {
    ...
    while let Some(dir) = current {
        ...
        let is_root = scan_roots.iter().any(|root| {
            ...
            fn normalize_path_str(p: &std::path::Path) -> String {
            ...
            normalize_path_str(dir) == normalize_path_str(root_path)
        });
...
fn is_generic_title(title: &str, episode: Option<u32>) -> bool {
    ...
    if t.chars().all(|c| c.is_numeric() || c.is_whitespace() || c == '0' || c == '-') {
        // If there is an episode number, check if the numeric value of the title
        // matches the episode number. If they match, it is generic.
        // Otherwise, it is a real title (like "86").
        if let Ok(num_val) = t.replace('-', "").trim().parse::<u32>() {
            if let Some(ep) = episode {
                if num_val == ep {
                    return true;
                }
            } else {
                // If there's no parsed episode but the value is very small, it might be generic
                if num_val < 50 {
                    return true;
                }
            }
...
fn best_title_for_file(
    ...
    if let Some(ref pt) = parsed.title {
        if !pt.trim().is_empty() && !is_generic_title(pt, parsed.episode) {
            return pt.clone();
        }
    }
...
pub fn scan_multiple(roots: &[String]) -> Result<Vec<ScannedSeries>, String> {
    ...
    /// Process a single file and insert it into the groups map.
    fn process_file(
        path_str: &str,
        file_name: &str,
        size: u64,
        groups: &mut BTreeMap<String, ScannedSeries>,
        roots: &[String],
    ) {
        let ext = crate::parser::file_ext(file_name);
        if !is_video(&ext) {
            return;
        }
        ...
        let raw_title = best_title_for_file(&parsed, path_str, roots);
    ...
    for root in roots {
        ...
        if root_path.is_file() {
            ...
            process_file(&path_str, &file_name, size, &mut groups, roots);
        } else {
...
mod tests {
    ...
    #[test]
    fn test_is_generic_folder() {
        assert!(!is_generic_folder("Working!!"));
        assert!(is_generic_folder("work 2"));
        assert!(!is_generic_folder("air"));
        assert!(is_generic_folder("eng"));
    }

    #[test]
    fn test_is_generic_title() {
        assert!(!is_generic_title("86", Some(1)));
        assert!(is_generic_title("01", Some(1)));
        assert!(is_generic_title("01", None));
        assert!(!is_generic_title("86", None));
    }

    #[test]
    fn test_find_best_folder_title() {
        let roots = vec!["C:/Anime/".to_string()];
        let title = find_best_folder_title("C:\\Anime\\86\\01.mkv", &roots);
        assert_eq!(title, Some("86".to_string()));
        
        let title_at_root = find_best_folder_title("C:\\Anime\\01.mkv", &roots);
        assert_eq!(title_at_root, None);
    }
}