...
import { Play, Calendar as CalendarIcon, Plus, Heart, Check, Tv } from "lucide-react";
...
export default function AiringCard({
  ...
  const [timeLeft, setTimeLeft] = useState(ep.airingAt - Math.floor(Date.now() / 1000));
  ...
  const timeStr = airDate.toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" });

  const statusText =
    timeLeft > 0
      ? `Airing in ${countdown(timeLeft)}`
      : `Aired at ${timeStr}`;
  ...
  const metaText = [
    [seasonStr, yearStr].filter(Boolean).join(" "),
    formatStr
  ].filter(Boolean).join(" · ");
    ...
    <div className="glass flex flex-col gap-3 rounded-2xl border border-border bg-surface p-4 text-left w-full h-full justify-between">
      ...
      <div className="w-full aspect-video rounded-xl overflow-hidden relative bg-white/5 border border-white/5 shrink-0">
        ...
        <div
          ...
          style={{ background: inLibrary ? "var(--accent)" : "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}
      ...
      <div className="flex-1 flex flex-col gap-1.5 min-w-0">
        {/* Title */}
        <h3 className="text-sm font-bold text-foreground leading-snug line-clamp-2" title={title}>
          {title}
        </h3>
      ...
      <div className="flex flex-col gap-2 w-full mt-1 shrink-0">
        ...
        <div className={`text-[11px] font-bold text-center ${timeLeft > 0 ? "text-yuui-accent3" : "text-muted-foreground"}`}>
          {statusText}
        </div>
      </div>
      ...
      <div className="border-t border-border pt-3 flex items-center justify-between w-full shrink-0">
        ...
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground font-semibold">
          ...
          <span>{ep.media.favourites ? ep.media.favourites.toLocaleString() : "0"}</span>
        </div>
      </div>
    </div>
  );
