...
export default function AnimeCard({
  ...
  const previewFile = entry.files.find((f) => f.video_preview != null);
  ...
  return (
    ...
    >
      <div
        className="absolute -inset-1 rounded-3xl opacity-0 blur-xl transition-opacity duration-300 group-hover:opacity-60"
      ...
      >
        <div className="relative aspect-[2/3] w-full overflow-hidden">
          ...
          <div className="absolute left-2 top-2 flex flex-wrap gap-1">
            {!entry.matched && (
              <span className="rounded-md bg-yellow-500/90 px-1.5 py-0.5 text-[10px] font-semibold text-black">
                REVIEW
              </span>
            )}
            {score != null && (
