...
/**
 * Quick view + inline editor for a library entry. Opened by right-clicking an
 * AnimeCard. Shows the full poster and every detail (all titles, format, dates,
 * source, studios, genres, tags, popularity, synopsis, local files) and lets
 * the user edit status / progress / score / notes / favorite without leaving
 * the library grid.
 */
export default function QuickViewModal({
  ...
  const m = (entry.media as any) || {};
  const a = (entry.analysis as any) || {};
  ...
  const files: any[] = [...((entry.files as any[]) || [])].sort((x, y) => (x.episode || 0) - (y.episode || 0));
  const size = ((entry.files as any[]) || []).reduce((s, f) => s + (f.size_bytes || 0), 0);
  const airing = m?.nextAiringEpisode;
  const episodeCount = m?.episodes ?? entry.episode_count ?? 0;
  ...
  useEffect(() => {
    const st = entry.user?.status;
    setEditStatus(!st || st === "Untracked" ? "Watching" : st);
    setEditProgress(entry.user?.progress || 0);
    setEditScore(entry.user?.score || 0);
    setEditNotes(entry.user?.notes || "");
    setEditFav(entry.user?.favorite || false);
  }, [entry.key]);
  ...
  return (
    <AnimatePresence>
      ...
      <div className="fixed inset-0 z-[61] grid place-items-center p-6 pointer-events-none">
        <motion.div
          ...
          <div className="flex-1 min-h-0 flex gap-6 px-7 pb-7 -mt-14">
            ...
            <div className="w-[236px] shrink-0 overflow-y-auto scrollbar-thin pr-1">
              ...
              <div className="mt-4 rounded-2xl bg-white/[0.02] border border-white/[0.04] px-3.5 py-2">
                ...
                <Fact label="Episodes" value={m?.episodes ?? entry.episode_count ?? "—"} mono />
            ...
            <div className="flex-1 min-w-0 overflow-y-auto scrollbar-thin pt-14 space-y-6">
              ...
              <div>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    onClick={() => { onClose(); navigate(`/anime/${encodeURIComponent(entry.key)}`); }}
              ...
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                <div className="flex items-center justify-between mb-3.5">
                  <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-yuui-accent">Quick Edit</span>
                  <AnimatePresence>
                    {justSaved && (
                ...
                <div className="grid grid-cols-[1fr_auto] gap-3 items-end">
                  <div>
                    ...
                    <select value={editStatus} onChange={(e) => setEditStatus(e.target.value)}
                      ...
                      {TRACK_STATUSES.map((s) => <option key={s} value={s} className="bg-black">{s}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[9px] font-bold uppercase tracking-widest text-white/30 mb-1.5 text-right">
                      Progress <span className="text-white/20">/ {episodeCount || "?"}</span>
                    </label>
                    <div className="flex items-center gap-1.5">
                      ...
                      <input type="number" value={editProgress}
                        onChange={(e) => setEditProgress(Math.max(0, Math.min(episodeCount || 9999, +e.target.value || 0)))}
                      ...
                      <button onClick={() => setEditProgress(Math.min(episodeCount || 9999, editProgress + 1))}
                        className="h-10 w-10 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.08] flex items-center justify-center text-white/70 text-lg font-light transition-colors">+</button>
                    </div>
                  </div>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2.5">
                  ...
                  <span className="text-[10px] font-mono text-white/35">{files.length} files · {fmtSize(size)}</span>
                </div>
