...
export function useAiringSchedule({
  ...
  // Local midnight, kept current across day rollovers — computed once per
  // mount it would keep showing yesterday as "Today" after midnight (and the
  // react-query key would never rotate).
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setStart((prev) => (prev === midnight ? prev : midnight));
    }, 60_000);
