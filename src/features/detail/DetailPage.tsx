...
export default function DetailPage() {
  ...
  const [activeVideo, setActiveVideo] = useState<{ path: string; episode: number; title: string } | null>(null);
  const [malData, setMalData] = useState<{ score: number | null; rank: number | null; popularity: number | null } | null>(null);
  const [tmdbArt, setTmdbArt] = useState<{ backdrop: string | null; logo: string | null } | null>(null);
  ...
  useEffect(() => {
    if (tmdbArt?.backdrop || banner) {
      setActiveBackdrop(tmdbArt?.backdrop || banner);
    }
    return () => {
      setActiveBackdrop(null);
    };
  }, [tmdbArt?.backdrop, banner, setActiveBackdrop]);
  ...
  // Fetch TMDB backdrop and logo dynamically if key is present
  useEffect(() => {
    let alive = true;
    ...
    const fetchTmdb = async () => {
      try {
        ...
        // Search TV show
        const searchUrl = `https://api.themoviedb.org/3/search/tv?api_key=${key}&query=${encodeURIComponent(title)}`;
        ...
        // Fetch TV images to look for logo
        const imagesUrl = `https://api.themoviedb.org/3/tv/${tvId}/images?api_key=${key}&include_image_language=en,ja,null`;
        const imagesResp = await fetch(imagesUrl);
        let logoPath = null;
        if (imagesResp.ok) {
          const imagesData = await imagesResp.json();
          const logo = imagesData.logos?.find((l: any) => l.file_path.endsWith(".png"));
          if (logo) {
        ...
        if (alive) {
          setTmdbArt({
            ...
            logo: logoPath ? `https://image.tmdb.org/t/p/original${logoPath}` : null,
          });
        }
      } catch (e) {
        // ignore
  ...
  if (!entry) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4">
        ...
        <p className="text-yuui-muted">That title couldn't be found.</p>
        <button
          onClick={() => navigate("/")}
    ...
    <div className="h-full overflow-y-auto">
      ...
      <div className="relative">
        <div className="relative h-[340px] w-full overflow-hidden">
          {tmdbArt?.backdrop || banner ? (
            <motion.img
              initial={{ scale: 1.08, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ duration: 0.8 }}
              src={tmdbArt?.backdrop || banner || undefined}
              alt=""
        ...
        <button
          onClick={() => navigate(-1)}
          className="glass absolute left-6 top-6 z-10 rounded-xl px-3 py-1.5 text-sm hover:bg-white/[0.1]"
        >
          ← Back
        </button>
            ...
            <div className="min-w-0 flex-1 pb-2">
              {tmdbArt?.logo ? (
                <motion.img
                  ...
                  src={tmdbArt.logo}
                  alt={title}
                  className="max-h-20 w-auto object-contain drop-shadow my-2 select-none pointer-events-none"
                />
              ) : (
                <motion.h1
                  initial={{ opacity: 0, y: 12 }}
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2.5">
                ...
                >
                  ...
                  {STATUS_OPTIONS.map((s) => (
                    <option key={s} value={s} className="bg-yuui-panel font-sans">
                      {s}
                    </option>
                  ))}
                </select>
                ...
                <select
                  value={user.score ?? ""}
                  onChange={(e) => update({ score: e.target.value ? Number(e.target.value) : null })}
                  className="glass rounded-xl bg-transparent px-3 py-2 text-sm outline-none border border-white/[0.05]"
                >
                  <option value="" className="bg-yuui-panel font-sans">Score…</option>
                ...
                <div className="glass flex items-center rounded-xl px-2.5 py-1 text-sm border border-white/[0.05] bg-white/[0.01]">
                  <span className="text-yuui-muted select-none px-2 font-medium text-xs uppercase tracking-wider font-sans">
                    Progress: {user.progress} / {media?.episodes ?? entry.episode_count}
                  </span>
                  <button
                    onClick={() => update({ progress: Math.max(0, user.progress - 1) })}
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (user.progress < max) {
                        ...
                        if (nextProgress === max && user.status !== "Completed") {
                          patch.status = "Completed";
                        }
                        update(patch);
                      }
                    }}
                    disabled={user.progress >= (media?.episodes ?? entry.episode_count)}
                    className="h-7 w-7 rounded-lg hover:bg-white/10 flex items-center justify-center font-bold text-white transition-colors disabled:opacity-30"
                  >
                    +
                  </button>
      ...
      <div className="px-6 pb-16">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {media!.genres.map((g) => (
                      <button
                        key={g}
                        onClick={() => navigate(`/discover?genre=${encodeURIComponent(g)}`)}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {tags.map((t) => (
                  <button
                    key={t.name}
                    onClick={() => navigate(`/discover?tag=${encodeURIComponent(t.name)}`)}
        ...
        {relations.length > 0 && (
          <Section title="Relations">
            <div className="flex gap-4 overflow-x-auto pb-2">
              {relations.map((r, i) => (
                <div key={`${r.node.id}-${i}`} className="w-[120px] shrink-0">
                  <div className="overflow-hidden rounded-xl border border-white/10">
                    <img
                      src={r.node.coverImage.extraLarge || r.node.coverImage.large || ""}
                      alt=""
                      className="aspect-[2/3] w-full object-cover"
                    />
                  </div>
        ...
        {recs.length > 0 && (
          <Section title="Recommended">
            <div className="flex gap-4 overflow-x-auto pb-2">
              {recs.map((r, i) => (
                <div key={`${r!.id}-${i}`} className="w-[120px] shrink-0">
                  <div className="overflow-hidden rounded-xl border border-white/10">
                    <img
                      src={r!.coverImage.extraLarge || r!.coverImage.large || ""}
                      alt=""
                      className="aspect-[2/3] w-full object-cover"
                    />
                  </div>
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex justify-end gap-1.5">
                        <button
                          onClick={() => setActiveVideo({ path: f.path, episode: f.episode || 0, title: f.file_name })}
      ...
      {activeVideo && (() => {
        ...
        const nextFile = entry.files.find((f) => f.episode === nextEpisodeNum);
        ...
        return (
          <VideoPlayerOverlay
            ...
            onWatched={async () => {
              const nextProgress = Math.max(user.progress, activeVideo.episode);
              const max = media?.episodes ?? entry.episode_count;
              const patch: Partial<UserData> = { progress: nextProgress };
              if (nextProgress === max && user.status !== "Completed") {
                patch.status = "Completed";
