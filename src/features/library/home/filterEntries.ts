 ...
 * from the `filteredAndSorted` useMemo.
 */
export function filterEntries(
  ...
  if (normalizedQuery) {
    ...
    parts.forEach((part) => {
      ...
      } else if (lower.startsWith("resolution:") || lower.startsWith("res:")) {
        const val = part.slice(part.indexOf(":") + 1).toLowerCase();
    ...
    list = list.filter((e) => {
      ...
      if (filters.text.length > 0) {
        const textMatch = filters.text.every((term) => {
          ...
          const inVA = (e.media?.characters?.edges as any[] | undefined)?.some(
            (edge) =>
      ...
      if (filters.resolutions.length > 0) {
        const matchesRes = filters.resolutions.every((fr) =>
          e.files?.some(
            (f: any) => f.resolution && f.resolution.toLowerCase().includes(fr),
          ),
        );
      ...
      if (filters.codecs.length > 0) {
        const matchesCodec = filters.codecs.every((fc) =>
          e.files?.some(
            (f: any) => f.codec && f.codec.toLowerCase().includes(fc),
          ),
        );
  ...
  if (statusFilter !== "ALL") {
    list = list.filter(
      (e) =>
        e.user?.status &&
        e.user.status.toUpperCase() === statusFilter.toUpperCase(),
    );
  }
  ...
  if (formatFilter !== "ALL") {
    list = list.filter(
      (e) =>
        e.media?.format &&
        e.media.format.toUpperCase() === formatFilter.toUpperCase(),
    );
  }
  ...
  if (currentTab === "COMPLETED") {
    ...
    list = list.filter((e) =>
      activePlaybackHistory.some(
        (h) =>
          (h.series_key && h.series_key === e.key) ||
          e.files.some((f: any) => h.file_path === f.path),
      ),
    );
