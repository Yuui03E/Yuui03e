...
export default function ActivityFeed({ userId }: ActivityFeedProps) {
  ...
  const loadActivities = async (pageNum: number, isLoadMore = false) => {
    ...
    try {
      const resp = await fetchUserActivities(graphqlAnilist, userId, pageNum);
  ...
  const getStatusText = (status: string, progress: string | null) => {
    ...
    if (s === "watched" || s === "completed" || s === "planning" || s === "dropped" || s === "paused") {
    ...
    <div className="glass rounded-[24px] p-4 bg-white/[0.01] border border-white/[0.05] h-full flex flex-col min-h-0 select-none overflow-hidden relative">
      ...
      {loading && page === 1 ? (
        ...
        <div className="flex-grow flex items-center justify-center text-center p-4">
          <span className="text-[11px] text-yuui-muted font-sans">No recent activities found.</span>
        </div>
      ) : (
        <div className="flex-grow overflow-y-auto space-y-2.5 pr-1 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
          {activities.map((activity: any) => {
            ...
            if (isList) {
              const mediaTitle = activity.media?.title?.english || activity.media?.title?.romaji || activity.media?.title?.userPreferred;
              return (
                <div
                  key={activity.id}
                ...
                >
                  <img
                    src={activity.media?.coverImage?.large || activity.media?.coverImage?.medium}
                    alt={mediaTitle}
                  ...
                  <div className="flex-grow flex flex-col justify-between min-w-0">
                    <div className="space-y-0.5 min-w-0">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        <span className="text-[9px] text-white/30 shrink-0 font-sans">{timeAgo}</span>
                      </div>
