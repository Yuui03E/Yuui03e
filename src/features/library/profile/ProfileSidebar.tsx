...
import { deleteMediaEntry } from "./api";
...
export default function ProfileSidebar({
  ...
  // Quick edit state via shared hook
  const {
    editStatus, setEditStatus,
    editProgress, setEditProgress,
    editScore, setEditScore,
  } = useQuickEdit({
    syncKey: selectedRowKey,
    getUserData: () => selectedEntry?.user ?? null,
    entryKey: null,
    mediaId: null,
  });
  ...
  const handleDeleteEntry = async () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to remove ${selectedEntry.title} from your AniList library?`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (!isNaN(listEntryId)) {
        await deleteMediaEntry(listEntryId);
      }
      ...
      const localEntry = entries.find(
        (le: any) => le.media?.id === selectedEntry.media.id,
      );
      if (localEntry) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      // Update online via AniList
      if (mediaId) {
        await saveMediaListEntry({ mediaId, progress: editProgress, status: editStatus, score: editScore });
      }
  ...
  return (
    <div className="col-span-3 flex flex-col gap-4 overflow-y-auto pr-1 h-full select-none">
      <div className="glass rounded-3xl p-3.5 border border-white/[0.05] bg-yuui-surface/40 space-y-1">
        <span className="text-[9px] font-bold text-yuui-muted uppercase tracking-wider block px-2.5 mb-1.5 font-display">
          Navigation
        </span>
            ...
            >
              {selectedEntry ? (
                ...
                <div className="glass rounded-3xl p-5 border border-yuui-accent bg-yuui-surface/60 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="text-yuui-muted hover:text-white cursor-pointer"
                    >
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                  ...
                  <div className="space-y-1 relative z-30">
                    <label className="text-[9px] text-yuui-muted uppercase font-bold tracking-wider">
                      Status
                    </label>
                      ...
                      {isDropdownOpen && (
                        <>
                          ...
                          <div className="absolute top-full left-0 right-0 mt-1 z-50 glass rounded-xl py-1 bg-[#10121a]/95 border border-white/[0.08] shadow-lg overflow-hidden">
                            {((selectedEntry?.media?.status ?? "FINISHED") ===
                            "FINISHED"
                              ? [...TRACK_STATUSES]
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="h-8 w-8 rounded-lg bg-white/5 border border-white/[0.05] hover:bg-white/10 flex items-center justify-center text-white cursor-pointer"
                      >
                        -
                      </button>
                      <input
                        ...
                        onChange={(e) =>
                          setEditProgress(
                            Math.min(
                              selectedEntry.episode_count,
                              Math.max(0, parseInt(e.target.value) || 0),
                            ),
                          )
                        }
                        className="flex-1 bg-[#11131a]/60 border border-white/[0.05] rounded-lg px-2 py-1.5 text-center text-xs text-white outline-none font-mono"
                      />
                      <button
                        onClick={() =>
                          setEditProgress(
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="flex-1 accent-yuui-accent cursor-pointer"
                      />
                      <span className="text-xs font-bold text-pink-400 font-mono w-8 text-right">
                        ★{editScore || "—"}
                      </span>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-yuui-muted uppercase tracking-wider block px-1">
                      Filter
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="space-y-1">
                      {["All", "Watching", "Completed", "Planning"].map(
                        (tabName) => {
                          const count =
                            tabName === "All"
                              ? stats.totalAnime
                          ...
                          return (
                            <div
                              ...
                              className={`flex items-center justify-between px-3 py-2 rounded-xl text-xs font-semibold cursor-pointer transition-all duration-200 ${
                                listFilter === tabName
                                  ? "text-white bg-yuui-accent/10 border-l-2 border-yuui-accent"
                                  : "text-yuui-muted hover:text-white hover:bg-white/[0.02]"
                              }`}
                            >
