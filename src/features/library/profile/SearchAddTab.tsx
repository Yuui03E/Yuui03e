...
export default function SearchAddTab({
  ...
  const debouncedGlobalSearch = useDebounce(globalSearchQuery, 400);

  // Debounced search trigger
  useEffect(() => {
    if (debouncedGlobalSearch.trim().length >= 3) {
      setSearchingGlobal(true);
      searchGlobalMedia(debouncedGlobalSearch)
        .then((resp) => setGlobalSearchResults(resp?.data?.Page?.media || []))
        .catch(console.warn)
        .finally(() => setSearchingGlobal(false));
    } else if (debouncedGlobalSearch.trim().length === 0) {
  ...
  const handleGlobalSearch = async () => {
    ...
    setSearchingGlobal(true);
    try {
      const resp = await searchGlobalMedia(globalSearchQuery);
