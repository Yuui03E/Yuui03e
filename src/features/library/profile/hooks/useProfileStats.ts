...
import { DEFAULT_EPISODE_DURATION } from "../../../../lib/format";
...
export function useProfileStats(
  ...
  const stats = useMemo(() => {
    ...
    const totalMinutes = activeEntries.reduce((sum, e) => sum + (e.user?.progress || 0) * (e.media?.duration || DEFAULT_EPISODE_DURATION), 0);
