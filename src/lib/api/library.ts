 ...
 * Unlike removeFolderEntries, this does an exact path comparison,
 * not a prefix match. Used for individual video files added via "Add Video Files". */
export async function removeFileEntries(
  filePath: string,
): Promise<StoredEntry[]> {
  return invoke<StoredEntry[]>("remove_file_entries", { filePath });
}

/** Copy text to the system clipboard via the Tauri backend. */
export async function copyToClipboard(text: string): Promise<void> {
  return invoke("copy_to_clipboard", { text });
}

/** Open a video file with the default system media player. */
export async function playVideoExternal(path: string): Promise<void> {
