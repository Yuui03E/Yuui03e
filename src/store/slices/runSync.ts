...
export async function runSync(
  ...
  try {
    ...
    unlistenSearching = await listen<SearchProgress>(
      "sync:searching",
      (event) => {
    ...
    unlistenComplete = await listen<{
      matched?: number;
      anilistSearch?: SearchProgress[];
      cancelled?: boolean;
    }>("sync:complete", (event) => {
      const payload = event.payload;
      // If cancelled, don't reset state (sync is being cancelled/aborted)
      if (payload.cancelled) {
        set({
          isSearching: false,
          isPaused: false,
          searchHistory: payload.anilistSearch ?? get().searchHistory,
        });
        return;
      }

      // Show toast with matched count if available
      if (typeof payload.matched === "number" && payload.matched > 0) {
        get().setToastMsg(`Synced ${payload.matched} series`);
        setTimeout(() => get().setToastMsg(null), 3000);
      }
    ...
    const finalEntries = await syncLibrary(folder, prune);
    const current = get().entries;
    const merged = finalEntries.map(
      (fe) => current.find((ce) => ce.key === fe.key) || fe,
    );
    set({
      entries: merged,
      status: "ready",
